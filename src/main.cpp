...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // inazuma: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "inazuma Signed Message:\n";
...
{
    // inazuma: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    // yearly decline of production by 7% per year, projected 213M coins max.
    for(int i = 210240; i <= nHeight; i += 210240) nSubsidy *= 0.93;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // inazuma: 1 day
static const int64 nTargetSpacing = 1.0 * 60; // inazuma: 1.0 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // inazuma: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, inazuma - DarkGravity, written by Evan Duffield - evan@inazuma.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, inazuma - DarkGravity v2, written by Evan Duffield - evan@inazuma.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // inazuma: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // inazuma: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "testbuild 13/5/2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1399975825;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1399975825;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // inazuma: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("inazumaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("inazumaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static inazumaMiner(CWallet *pwallet)
{
    printf("inazumaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("inazuma-miner");
    ...
    try { loop {
        ...
        printf("Running inazumaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("inazumaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&inazumaMiner, pwallet));
}
